...
p.means_pe = pe_task.means %>% 
  ...
  facet_wrap(relation ~ world, scales = "free_x", ncol = 4, labeller = label_both) +
  labs(y = "Mean estimates across participants", x = "condition (trial)")
p.means_pe
ggsave(paste(plot_dir, "mean-estimates-pe-task.png", sep = FS), 
       p.means_pe, width = 14, height = 9)
```
...
print(paste('nb participants who gave custom response:', round(ratio_subjs, 3)))
...
p.utt_freq = df.utt_freq %>% ggplot(aes(y = utt_f, x = n)) + 
  ...
  labs(y = "utterance", x = "number selections")
p.utt_freq
...
# dependent cases: both fall, neither falls, if blue green, if not blue not green
df.dep_cost = uc_data %>%
  mutate(cost.uc = factor(cost.uc, levels = seq(2, 7))) %>% 
...
df.dep_cost.sum = df.dep_cost %>% 
  group_by(utt.standardized, utt_type, cost.uc, uc_task) %>% 
  summarize(n = n(), .groups = "drop_last") %>% arrange(desc(n))
df.dep_cost.sum$utt_f = factor(df.dep_cost.sum$utt.standardized, 
                           levels = c(df.dep_cost.sum$utt.standardized %>% unique()))
  
p.dep_cost = df.dep_cost.sum %>% 
  ggplot(aes(x=n, y = utt_f, fill = cost.uc)) + 
  geom_bar(stat = "identity") + 
  scale_fill_brewer(name = "utterance cost (number of clicks)", palette = 'OrRd') +
  labs(x = "number selections", y = "selected utterance (standardized)") +
  guides(fill = guide_legend(nrow = 1))
p.dep_cost


ggsave(paste(plot_dir, "dep_cost.png", sep = FS), p.dep_cost, width = 6, height = 3)
```
...
trials_remove = df.utt_freq_by_rel %>% group_by(relation) %>% 
  mutate(N = sum(n), ratio = round(n / N, 2) * 100) %>% 
...
# only consider conditional `if blue falls green falls' for relations where s
